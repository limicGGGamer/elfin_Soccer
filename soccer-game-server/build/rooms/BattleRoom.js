...
class BattleRoom extends core_1.Room {
            ...
            switch (type) {
                ...
                case "player-state":
                    if (message?.type == "getLatestState") {
                        ...
                        client.options = { userId: message?.data?.userId };
                        const syncTicketData = {
                            "userId": message?.data?.userId,
                            "ticket_id": _player?.player?.ticket,
                            "state": "battle",
                            "game_id": "MantaDao",
    ...
    async setPlayer(playerstate) {
        ...
        Object.entries(playerstate?.player).forEach(([sessionId, options], index) => {
            const _player = this.state.createPlayer(options?.sessionId, options, index, options?.uid, "battle", options?.walletid, options?.ticket, options?.passCred, options?.skinId, options?.playerNumber);
            //this.broadcast("game-event", { event: `set-player${index + 1}`, data: { player: options, sessionId: (options as { uid: string })?.uid, figther: index} });
            this.state.createSangPad(options?.sessionId);
            this.broadcast("game-event", { event: `set-player`, data: { sessionId: options?.sessionId,
                    skinId: options?.skinId, playerNumber: options?.playerNumber } });
        });
